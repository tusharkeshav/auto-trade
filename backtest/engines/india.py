# ─────────────────────────────────────────────────────────────────
#  backtest/engines/india.py
#  Walk-forward backtesting engine for NSE/BSE instruments.
#
#  Strategy: IndiaSignalScorer (3-gate mean-reversion + VIX + ADX)
...
#      bt = IndiaBacktestEngine("NIFTY50", "15m", bars=1400)
#      result = bt.run()
...
class IndiaBacktestTrade:
    ...
    entry_price:  float
    exit_price:   float
    qty:          float      # units (fractional for indices)
...
class IndiaBacktestResult:
    ...
    trades:          list[IndiaBacktestTrade] = field(default_factory=list)
    ...
    @property
    def winning_trades(self) -> list[IndiaBacktestTrade]:
        return [t for t in self.trades if t.pnl > 0]

    @property
    def losing_trades(self) -> list[IndiaBacktestTrade]:
        return [t for t in self.trades if t.pnl <= 0]
...
class IndiaBacktestEngine:
    """
    Walk-forward backtesting engine for NSE instruments (NIFTY/BANKNIFTY).

    Parameters
    ----------
    symbol      : NSE yfinance symbol, e.g. "NIFTY50" or "BANKNIFTY"
    interval    : Candle timeframe — "15m", "1h", "1d"
    bars        : Historical candles to test on
    threshold   : Min probability for LONG entry
    atr_sl_mult : SL = atr_sl_mult × ATR from entry
    atr_tp_mult : TP = atr_tp_mult × SL distance (R-multiple)
    capital     : Starting capital in INR
    vix         : India VIX at backtest start (refreshed per candle if passed as df column)
    max_timeout : Max candles to hold before forced exit (0 = no timeout)
    """
    ...
    def run(self, external_df: pd.DataFrame | None = None) -> IndiaBacktestResult:
        df = external_df if external_df is not None else self._load_data()

        WARMUP = min(200, len(df) // 4)

        result = IndiaBacktestResult(
            symbol          = self.symbol,
        ...
        logger.success(f"Backtest done: {result.total_trades} trades, "
    ...
    def _make_trade(
        ...
        # Transaction costs: entry + exit legs
    ...
    def print_report(self, r: IndiaBacktestResult) -> None:
        ...
        def sep():
            c.print("[bold blue]" + "═" * 68 + "[/]")

        sign    = "+" if r.total_pnl >= 0 else ""
        pnl_col = "green" if r.total_pnl >= 0 else "red"

        sep()
        c.print(f"[bold cyan]  INDIA BACKTEST — {r.symbol}  {r.interval}[/]")
        sep()
        ...
        for trade in r.trades[-20:]:
            ...
            tbl.add_row(
                ...
                f"[{dc}]{trade.direction}[/]",
                f"₹{trade.entry_price:>8,.1f}",
                f"₹{trade.exit_price:>8,.1f}",
                trade.exit_type,
                f"[{pc}]{sgn}₹{trade.pnl:>8,.2f}[/]",
