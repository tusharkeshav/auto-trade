# ─────────────────────────────────────────────────────────────────
#  backtest/engines/vcp_breakout_engine.py
#  Standalone Minervini Volatility Contraction Pattern (VCP) Engine.
#
#  Mathematical & Quantitative Architecture:
#    1. Macro Filter: NIFTY 50 (^NSEI) > SMA200.
#    2. Trend Template: Stock Close > SMA200 and Stock Close > SMA50.
#    3. Volatility Contraction: Bollinger Bandwidth < 0.10 over 60 bars.
#    4. Volume Confirmation: Volume Ratio >= 1.15 (15%+ volume expansion).
#    5. Pivot Breakout: Close >= 20-Day High * 0.999 & Prev Close < Prev 20-Day High.
#    6. Asymmetric Risk: Stop Loss = 1.25 × ATR, Take Profit = 3.50 × ATR.
#    7. Trailing Profit Lock: +0.5R locked when trade reaches +2.0R.
#    8. Exact Indian CNC Delivery Statutory Tax Deductions.
# ─────────────────────────────────────────────────────────────────
...
class VCPBreakoutEngine:
    """
    Standalone Minervini Volatility Contraction Pattern (VCP) Quantitative Engine.
    """
    ...
    def run(self, bars: int = 1250) -> VCPResult:
        ...
        logger.info(f"Fetching {bars} Daily candles for VCP Universe ({len(all_syms)} assets)...")
        ...
        for t in range(warmup, total_bars):
            ...
            for sym in list(open_positions.keys()):
                ...
                # Trailing Profit Lock (+1.0R lock when price reaches +2.0R)
            ...
            # ── 2. Scan for Minervini VCP Breakouts ──
            if macro_bull and len(open_positions) < self.max_open_trades:
                for sym in self.symbols:
                    ...
                    # 3. VCP Condition: Uptrend + Fresh Breakout + Volatility Squeeze (<0.10) + Vol Expansion (>1.15x)
