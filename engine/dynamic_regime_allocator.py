# ─────────────────────────────────────────────────────────────────
#  engine/dynamic_regime_allocator.py
#  Dynamic Macro-Regime Capital Allocator Engine.
#
#  Mathematical Allocation Logic:
#    1. Macro Gate: NIFTY 50 (^NSEI) > SMA200.
#    2. Trend Strength: ADX(14) >= 22.0 and EMA12 > EMA50.
#    3. Dynamic Capital Weights:
#       - TRENDING_BULL : 60% Sector ETF Momentum + 40% Large-Cap Stocks
#       - CHOPPY_RANGE  : 70% Large-Cap Pullbacks (60d RS Gate) + 30% Gold
#       - BEAR_DEFENSE  : 100% Sovereign Gold (GOLDBEES) / Liquid Cash
# ─────────────────────────────────────────────────────────────────
...
class DynamicRegimeAllocator:
    ...
    def run(self, bars: int = 1250) -> DynamicRegimeResult:
        # 1. Fetch Benchmark and Gold Data
        ...
        eng_pullback = MasterPortfolioEngine(routing_map=STOCK_ROUTING_MAP, interval="1d", bars=bars, capital=self.total_capital, max_positions_per_master=3, max_open_trades=6)
        res_pullback = eng_pullback.run()
        ...
        for t in sorted(res_pullback.trades, key=lambda x: x.exit_time):
