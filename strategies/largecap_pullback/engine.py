# ─────────────────────────────────────────────────────────────────
#  strategies/largecap_pullback/engine.py
#  Large-Cap Pullback Engine with 60-Day Relative Strength (RS) Gate.
# ─────────────────────────────────────────────────────────────────
...
class LargeCapPullbackEngine:
    def __init__(
        ...
        self.use_rs_gate        = use_rs_gate

    def run(self, bars: int = 1250) -> PullbackResult:
        ...
        for t in range(warmup, total_bars):
            ...
            if macro_bull and len(open_positions) < self.max_open_trades:
                for sym in self.symbols:
                    ...
                    # 60d RS Gate
                    rs_pass = True
                    if self.use_rs_gate and t >= 60:
                        rs_today = px / float(bm_series.iloc[t])
                        rs_60 = float(df_s["close"].iloc[t-60]) / float(bm_series.iloc[t-60])
                        rs_slope = ((rs_today - rs_60) / rs_60) * 100.0
                        rs_pass = rs_slope > 0.0

                    is_pullback = (prev_px <= prev_sma20 * 1.005) and (px > sma20) and (40.0 <= rsi <= 58.0) and rs_pass

                    if is_pullback and capital > 1000:
