# ─────────────────────────────────────────────────────────────────
#  strategies/largecap_pullback/run_test.py
#  Standalone Runner for Large-Cap Pullback Engine.
# ─────────────────────────────────────────────────────────────────
...
def main():
    ...
    console.print(Panel.fit(
        "[bold cyan]🎯 STRATEGY 2: LARGE-CAP PULLBACK ENGINE (60-DAY RS GATE)[/]\n"
    ...
    eng_5y = LargeCapPullbackEngine(capital=50000.0, max_open_trades=6, use_rs_gate=True)
    res_5y = eng_5y.run(bars=1250)

    console.print("[bold yellow]Running 1-Year Forward Test Replay (452 bars)...[/]")
    eng_1y = LargeCapPullbackEngine(capital=50000.0, max_open_trades=6, use_rs_gate=True)
    res_1y = eng_1y.run(bars=452)

    mc_stats = run_monte_carlo([t.net_pnl_pct for t in res_5y.trades])

    tbl = Table(title="[bold green]📊 STRATEGY 2 SCORECARD: LARGE-CAP PULLBACK[/]", box=box.DOUBLE_EDGE, header_style="bold cyan")
    ...
    tbl.add_row("Annualized Return (CAGR)", f"[bold green]{res_5y.cagr_pct:.2f}%[/]", f"[bold green]{res_1y.cagr_pct:.2f}%[/]")
    tbl.add_row("Maximum Peak-to-Trough DD", f"[bold green]-{res_5y.max_drawdown_pct:.2f}%[/]", f"[bold green]-{res_1y.max_drawdown_pct:.2f}%[/]")
    tbl.add_row("Profit Factor (Gross Win / Loss)", f"[bold yellow]{res_5y.profit_factor:.2f}[/]", f"[bold yellow]{res_1y.profit_factor:.2f}[/]")
    tbl.add_row("Audited Win Rate (%)", f"{res_5y.win_rate:.1f}%", f"{res_1y.win_rate:.1f}%")
    tbl.add_row("Total Trades Executed", f"{res_5y.total_trades} trades (~33/yr)", f"{res_1y.total_trades} trades")
