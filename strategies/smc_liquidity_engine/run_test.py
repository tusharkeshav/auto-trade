# ─────────────────────────────────────────────────────────────────
#  strategies/smc_liquidity_engine/run_test.py
#  Standalone Runner for Smart Money Concepts (SMC) Engine.
# ─────────────────────────────────────────────────────────────────
...
def main():
    ...
    console.print(Panel.fit(
        "[bold cyan]🏦 STRATEGY 6: SMART MONEY CONCEPTS (SMC) QUANTITATIVE ENGINE[/]\n"
    ...
    eng_5y = SMCLiquidityEngine(capital=50000.0, max_open_trades=6)
    res_5y = eng_5y.run(bars=1250)

    console.print("[bold yellow]Running 1-Year Forward Test Replay (452 bars)...[/]")
    eng_1y = SMCLiquidityEngine(capital=50000.0, max_open_trades=6)
    res_1y = eng_1y.run(bars=452)

    mc_stats = run_monte_carlo([t.net_pnl_pct for t in res_5y.trades])

    tbl = Table(title="[bold green]📊 STRATEGY 6 SCORECARD: SMART MONEY CONCEPTS (SMC)[/]", box=box.DOUBLE_EDGE, header_style="bold cyan")
    ...
    tbl.add_row("Total Trades Executed", f"{res_5y.total_trades} trades (~42/yr)", f"{res_1y.total_trades} trades")
