# ─────────────────────────────────────────────────────────────────
#  strategies/vcp_breakout/run_test.py
#  Standalone Runner for Minervini VCP Breakout Engine.
# ─────────────────────────────────────────────────────────────────
...
def main():
    ...
    console.print(Panel.fit(
        "[bold cyan]🚀 STRATEGY 3: MINERVINI VCP BREAKOUT ENGINE[/]\n"
    ...
    eng_5y = VCPBreakoutEngine(capital=50000.0, max_open_trades=6)
    res_5y = eng_5y.run(bars=1250)

    console.print("[bold yellow]Running 1-Year Forward Test Replay (452 bars)...[/]")
    eng_1y = VCPBreakoutEngine(capital=50000.0, max_open_trades=6)
    res_1y = eng_1y.run(bars=452)

    mc_stats = run_monte_carlo([t.net_pnl_pct for t in res_5y.trades])

    tbl = Table(title="[bold green]📊 STRATEGY 3 SCORECARD: MINERVINI VCP BREAKOUT[/]", box=box.DOUBLE_EDGE, header_style="bold cyan")
    ...
    tbl.add_row("Total Trades Executed", f"{res_5y.total_trades} trades (~18/yr)", f"{res_1y.total_trades} trades")
